    ...
    async def send(
        ...
        *,
            ...
            if anonymous and from_mod and not isinstance(destination, discord.TextChannel):
                ...
                if ticket_message:
                    ...
                    if getattr(message, "_menu_invoked", False):
                        ...
                        avatar_url = getattr(self.bot.user.display_avatar, "url", avatar_url)
                    else:
                        name = str(author)
        ...
        if plain:
            ...
            try:
                msg = await destination.send(mentions, embed=embed, files=files)
                await self.handle_ticket_message(msg, embed, ticket_message, receive_reply)
            except discord.NotFound:
                if (
